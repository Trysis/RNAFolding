...
SELECT_MODELS = {
    ...
    "build_lstm_model2": model.build_lstm_model2
}
...
def load_model(model_link,
    ...
    model_name, model = "unknown.tf", None
...
def train_model(model_link, x_train, y_train, x_val=None, y_val=None,
    ...
    if not all([x_val is None, y_val is None]) and any([x_val is None, y_val is None]):
        raise Exception("When x_val or y_val is set, both should be provided.")
    ...
    # Check if at least one callback is applied
...
if __name__ == "__main__":
    ...
    parser.add_argument('-e', '--epochs', type=int, default=100, help="number of epochs to perform")
